/**
 * Auto-generated from OpenDota dotaconstants
 * Source: https://github.com/odota/dotaconstants/blob/master/build/item_ids.json
 * Last updated: 2024-12-29
 */

// Item ID to internal name mapping
...
export function getItemImageUrl(itemId: number): string | null {
    ...
    if (!itemName || itemId === 0) return null;
    return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/items/${itemName}.png`;
}

// Get display name (convert internal_name to Title Case)
export function getItemDisplayName(itemId: number): string {
    const itemName = ITEM_IDS[itemId];
    if (!itemName) return `Item #${itemId}`;
    const displayName = itemName.replace(/^recipe_/, "");
    return displayName
        .split("_")
...
// For compatibility with Scoreboard which uses item name strings
